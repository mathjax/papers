...
\cite{MJ-input}, and severl output formats, including HTML-with-CSS,
...
MathJax can make your on-line course materials or published research
...
MathJax has been undergoing a complete rewrite form the ground up,
...
In order to make MathJax easier to maintain, version 3 is written in
the Typescript language \cite{Typescript}, a version of javascript
that includes information about the type of information stored in each
variable and returned by each function.  This allows errors to be
found much earlier, leads to more reliable code that is easier to
understand (and so easier for others to contribute to the MathJax).
It also allows us to use new features of javascript that are part of
the ES6 standard \cite{ES6}, while still supporting older browsers
that don't implement them.  So, for example, we can take advantage of
ES6 promises \cite{ES6-promise} rather than MathJax v2's custom
signals, queues, and callbacks that many found difficult to use, and
ES6's modern class structure \cite{ES6-class} rather than the
non-standard approach found in v2.
...
components, which did not work well with javascript packaging systems
